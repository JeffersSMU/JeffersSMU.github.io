function generate_board() {
    ...
    // YOUR CODE GOES HERE
    for (friend of document.getElementsByTagName("option")) {
        if (friend.selected) {
    ...
    if (friends.length > 0) {
        ...
        for (fruit of fruits) {
            for (friend of friends) {
                card_array.push(`${fruit}_${friend}.png`);
                card_array.push(`${fruit}_${friend}.png`);
            }
        }
    ...
    for (let row = 0; row < num_rows; row++) {
        ...
        for (let col = 0; col < num_cols; col++) {
            result_str += `<td class="column"><img src="./cards/${card_array[row*num_cols + col]}"></td>`;
        }
        result_str += `</tr>`;
    }
    result_str += `</table>`;
